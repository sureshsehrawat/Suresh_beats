...
import { getRecentAlbums, getCoverArtUrl } from '../lib/subsonic';
...
export function RecentAlbums({ onAlbumClick }: RecentAlbumsProps) {
  ...
  useEffect(() => {
    const fetchAlbums = async () => {
      try {
        ...
        setError(error instanceof Error ? error.message : 'Failed to fetch recent albums');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="relative">
      <h3 className="text-lg font-semibold mb-4 text-zinc-400">Recently Added</h3>
